...
export function resolveValuesDir(): string {
  if (process.env.VALUES_DIR) {
    return join(process.env.VALUES_DIR, CONTRACT_VERSION);
  }
  return resolve(__dirname, "../../../../values", CONTRACT_VERSION);
}

export function loadAndValidate(valuesDir = resolveValuesDir()): LoadResult {
